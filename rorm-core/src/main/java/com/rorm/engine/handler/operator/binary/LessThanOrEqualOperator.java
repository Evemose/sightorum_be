...
public final class LessThanOrEqualOperator implements BuiltInBinaryOperatorHandler {

    public static final String NAME = "LESS_THAN_OR_EQUAL";
    ...
    public Field<?> transform(Expression left, Expression right, TransformContext ctx) {
        return ((Field) ctx.transform(left)).le(ctx.transform(right));
    }
}

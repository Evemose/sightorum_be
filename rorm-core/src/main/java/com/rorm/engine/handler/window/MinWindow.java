...
public final class MinWindow extends AbstractWindowFunction {
    ...
    public Field<?> transform(List<Expression> args, WindowSpec windowSpec, TransformContext ctx) {
        ...
        return applyWindowSpec(DSL.min(ctx.transform(args.getFirst())), partition, order, windowSpec.frame());
    }
}
